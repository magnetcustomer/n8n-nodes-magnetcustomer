...
describe('Customer E2E', () => {
  ...
  it('create', async () => {
    const result = await run(wb.customerCreate());
  ...
  it('get', async () => {
    const result = await run(wb.getById('customer', 'customerId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  ...
  it('getAll', async () => {
    const result = await run(wb.getAll('customer'));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('search', async () => {
    const result = await run(wb.search('customer', getConfig().options.cleanupPrefix));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('update', async () => {
    const result = await run(wb.contactUpdate('customer', 'customerId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('delete', async () => {
    const result = await run(wb.deleteById('customer', 'customerId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
});
