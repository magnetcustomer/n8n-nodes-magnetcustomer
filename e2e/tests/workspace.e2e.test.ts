...
describe('Workspace E2E', () => {
  ...
  it('create', async () => {
    const result = await run(wb.workspaceCreate());
  ...
  it('get', async () => {
    const result = await run(wb.getById('workspace', 'workspaceId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  ...
  it('getAll', async () => {
    const result = await run(wb.getAll('workspace'));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('search', async () => {
    const result = await run(wb.search('workspace', getConfig().options.cleanupPrefix));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
  ...
  it('delete', async () => {
    const result = await run(wb.deleteById('workspace', 'workspaceId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
});
