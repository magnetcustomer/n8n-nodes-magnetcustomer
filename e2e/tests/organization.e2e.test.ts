...
describe('Organization E2E', () => {
  ...
  it('create', async () => {
    const result = await run(wb.organizationCreate());
  ...
  it('get', async () => {
    const result = await run(wb.getById('organization', 'organizationId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  ...
  it('getAll', async () => {
    const result = await run(wb.getAll('organization'));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('search', async () => {
    const result = await run(wb.search('organization', getConfig().options.cleanupPrefix));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
  ...
  it('delete', async () => {
    const result = await run(wb.deleteById('organization', 'organizationId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
});
