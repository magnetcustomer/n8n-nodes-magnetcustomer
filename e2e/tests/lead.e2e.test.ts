...
describe('Lead E2E', () => {
  ...
  it('create', async () => {
    const result = await run(wb.leadCreate());
  ...
  it('get', async () => {
    const result = await run(wb.getById('lead', 'leadId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  ...
  it('getAll', async () => {
    const result = await run(wb.getAll('lead'));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });

  it('search', async () => {
    const result = await run(wb.search('lead', getConfig().options.cleanupPrefix));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
  ...
  it('delete', async () => {
    const result = await run(wb.deleteById('lead', 'leadId', recordId));
    expect(result.status).toBe('success');
    expect(result.output).toHaveLength(1);
  });
});
